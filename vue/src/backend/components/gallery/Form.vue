    ...
    >
      <div class="shadow sm:overflow-hidden sm:rounded-md">
        <div class="space-y-10 bg-white sm:p-6">
          ...
          <!-- <pre>{{ model }}</pre> -->
          <div>
            <label class="block font-medium text-gray-700">Gallery Image</label>
            <div
...
watch(
  () => store.state.gallery.current.data,
  (newVal, oldVal) => {
    model.value = {
...
if (route.params.id) {
  store.dispatch("gallery/getById", route.params.id);
  action = "updated";
}

const save = () => {
  store
    .dispatch("gallery/saveImage", model.value)
    .then(({ data }) => {
      store.commit("notify", {
        type: "success",
